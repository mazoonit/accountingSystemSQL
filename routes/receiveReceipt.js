...
var receiveReceipts=require('../decryptoric/receiveReceipts.js');
...
router.post('/frontGet',[basicAuth,detailedAdvancedAuth.cashRead],function(req,res,next){
  receiveReceipts.getReceipt(req.body.id,req.body.type).then((receiptObj)=>{
    if(!receiptObj){
...
router.post('/',[basicAuth,detailedAdvancedAuth.cashAdd],function(req,res,next){
    ...
    receiveReceipts.addReceipt(receiptObj).then((result)=>{
...
router.put('/',[basicAuth,detailedAdvancedAuth.cashEdit],function(req,res,next){
  ...
  receiptObj.userId=req.session.user.id;
  receiveReceipts.editReceipt(receiptObj,type).then((result)=>{
    if(result){
...
//   receiveReceipts.deleteReceipt(req.body.id,req.body.type).then((result)=>{
...
// router.get('/getPendingreceiveReceipts',function(req,res,next){
